  ...
  const register = async () => {
    ...
    try {
      ...
      if (response.ok || data.status == 200) {
        handleSubmit();
        console.log("Success:", { status: 200, ok: true });
      } else {
        toast(`❌  Failed to Join Starel WaitList`, {
          position: "bottom-center",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: false,
        ...
        console.log("Failed:", { status: 400, ok: false });
      }
    } catch (error) {
      ...
      setLoading(false); // Hide loading component regardless of success or failure
